...
*Register an Item and release it to Legal entity (USMF)*

| 항목 | 내용 |
|---|---|
| **메뉴 경로** | `Product information management > Products > All products > New`<br>`→ Release products (Legal entity: USMF 체크)` |
| **목적** | 사고팔 대상 품목을 등록하고, USMF 법인에서 실제로 사용 가능하도록 릴리즈. 릴리즈 전에는 PO/SO에서 선택 불가. |

**[ 스크린샷 삽입 위치 ]**

**설명 / 확인 포인트:** All products에는 보이는데 Released products에는 안 보이면 릴리즈 누락. Release products 액션 실행 후 재확인.
...
| 항목 | 내용 |
|---|---|
| **메뉴 경로** | `Procurement and sourcing > Purchase orders > All purchase orders > New`<br>`→ Vendor 선택, 라인 입력 → Confirm` |
| **목적** | 벤더에게 보낼 공식 주문서를 생성하고 확정. Confirm 시점까지는 재고/회계에 영향 없음 ("주문서만 던진" 상태). |

**[ 스크린샷 삽입 위치 ]**

**설명 / 확인 포인트:** Confirm 직후 PO 상태(Approval status / Purchase order status) 확인. 아직 InventTrans나 Voucher가 생성되지 않았는지 확인.
...
**설명 / 확인 포인트:** Inventory management > Inquiries > Transactions에서 방금 생성된 InventTrans 확인.

---

### Step 6. Vendor Invoice 처리 (3-way matching)
*Process Vendor Invoice with Three-way matching*

| 항목 | 내용 |
|---|---|
| **메뉴 경로** | `PO 화면 > Invoice > Vendor invoice`<br>`→ PO-입고-인보이스 수량/금액 대사 → Post` |
| **목적** | 벤더 청구서를 PO 및 입고 내역과 대사하여 확정. 이 시점에 매입채무(AP)가 실제로 확정되고, 입고 시 잠정계정이 정리됨. |

**[ 스크린샷 삽입 위치 ]**

**설명 / 확인 포인트:** Post 후 Voucher transaction 확인하여 AP 계정에 금액이 실제로 잡히는지 확인.

---

### Step 7. Vendor Payment 및 Settle
*Vendor Payment Journal / Payment Proposal → Settle*

| 항목 | 내용 |
|---|---|
| **메뉴 경로** | `Accounts payable > Payments > Payment journal > New`<br>`→ Line 입력 → Post → Settle` |
| **목적** | 실제 자금 지급을 처리하고 매입채무를 청산. Settle 후 Vendor의 미지급 잔액이 0으로 정리됨. |

**[ 스크린샷 삽입 위치 ]**

**설명 / 확인 포인트:** Vendor 화면에서 Balance(잔액)이 지급 전/후 어떻게 달라지는지 비교.
